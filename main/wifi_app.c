...
/**
 * Configures the WiFi AP setttings and assigns the static IP to the SoftAP.
 */
static void wifi_app_soft_ap_config(void) {
  ...
  memset(&ap_ip_info, 0x00, sizeof(ap_ip_info));                              // we want this cleared out first

  esp_netif_dhcps_stop(esp_netif_ap);                                         // must call this first!
  inet_pton(AF_INET, WIFI_AP_IP, &ap_ip_info.ip);                             // assigns access point's staic IP, gateway, and netmask
  ...
  ESP_ERROR_CHECK(esp_netif_dhcps_start(esp_netif_ap));                       // Starts the AP DHCP server (for connecting stations, e.g., your mobile device)
...
/**
 * Initializes the WiFi application handler for WiFi and IP events
 */
...
static void wifi_app_task(void *pvParameters) {
  ...
  wifi_app_send_message(WIFI_APP_MSG_START_HTTP_SERVER); // Send the first event message
...
void wifi_app_start(void) {
  ...
  rgb_led_wifi_app_started();                                                // Start WiFi started LED
  esp_log_level_set("wifi", ESP_LOG_NONE);                                   // Disable default WiFi logging messages
  wifi_app_queue_handle = xQueueCreate(3, sizeof(wifi_app_queue_message_t)); // Create message queue
